...
@interface NoAssessViewController ()<UITableViewDataSource, UITableViewDelegate,OrderTableViewCellDelegate>
...
@implementation NoAssessViewController{
    int pageIndex;
}
...
-(void)loadOrderData{
    ...
    NSURL *url = [NSURL URLWithString: [NSString stringWithFormat: KOrder_List_URL,@"3",pageIndex,uid,token ] ];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url ];
    request.timeoutInterval=KHTTPTimeoutInterval;
    [request setHTTPMethod:@"GET"];
    
    __block NoAssessViewController *weakSelf = self;
