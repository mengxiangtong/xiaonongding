...
@interface NoCompleteViewController ()<UITableViewDataSource, UITableViewDelegate,OrderTableViewCellDelegate>
...
@implementation NoCompleteViewController{
    int pageIndex;
}
...
-(void)loadOrderData{
    ...
    NSURL *url = [NSURL URLWithString: [NSString stringWithFormat: KOrder_List_URL,@"4",pageIndex,uid,token ] ];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url ];
    request.timeoutInterval=KHTTPTimeoutInterval;
    [request setHTTPMethod:@"GET"];
    
    __block NoCompleteViewController *weakSelf = self;
