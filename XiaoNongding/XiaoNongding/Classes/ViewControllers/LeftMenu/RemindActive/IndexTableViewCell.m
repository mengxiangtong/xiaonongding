...
//    [bgView addSubview:_lb_state];
    ...
    //
    UILabel *la = [[UILabel alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(l2.frame)+10, 80, 30)];
    ...
    //3
    self.lb_Price=[[UILabel alloc]initWithFrame:CGRectMake(15+CGRectGetMaxX(la.frame) , CGRectGetMaxY(l2.frame)+10, 100.0, 30.0)];
    ...
    [_Btn_One addTarget:self action:@selector(goPay:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)setDic_Item:(NSDictionary *)dic_Item{
    _dic_Item=dic_Item;
    if (_dic_Item) {
        NSString *farm_Name=[dic_Item objectForKey:@"farm_name"];
        if (![farm_Name isEqual:[NSNull null]] && farm_Name) {
            self.lb_farm.text =farm_Name;
        }else
            self.lb_farm.text =@"";
        ...
        NSString *pic=[dic_Item objectForKey:@"pic"];
        if (![ pic isEqual:[NSNull null] ] && pic ) {
            [self.img_Product sd_setImageWithURL:[NSURL URLWithString:pic] placeholderImage:[UIImage imageNamed:@"index_wcl"]];
        }
       NSString*name= [dic_Item objectForKey:@"name"];
        if (![name isEqual:[NSNull null]] && name) {
             self.lb_Title.text=name;
        }else
            self.lb_Title.text=@"";
        ...
        NSString *end_time=[dic_Item objectForKey:@"end_time"];
        if (![end_time isEqual:[NSNull null]] && end_time) {
             self.lb_Date.text=[NSString stringWithFormat:@"截止时间: %@",[SO_Convert DateToString:[NSDate dateWithTimeIntervalSince1970:[end_time integerValue]] DateFormat:@"yyyy-MM-dd HH:mm:ss"]];
        }else
             self.lb_Date.text=@"截止时间: ";
       
        NSString *price=[NSString stringWithFormat:@"¥%@", [dic_Item objectForKey:@"money"] ];
        if (![price  isEqual:[NSNull null]] && price) {
            self.lb_Price.text=[dic_Item objectForKey:@"money"];
        }else
            self.lb_Price.text=@"";
        
        NSString *typeStr=[dic_Item objectForKey:@"type"];
        if (![typeStr isEqual:[NSNull null]] &&[typeStr isEqualToString:@"1"]) {
            self.lb_state.text=@"";
        }else{
            self.lb_state.text=@"";
        }
