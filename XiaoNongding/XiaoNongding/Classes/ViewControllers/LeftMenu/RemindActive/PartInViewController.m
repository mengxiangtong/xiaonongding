...
@interface PartInViewController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation PartInViewController{
    int pageIndex;
}
...
+(instancetype )shareInstance
{
    ...
    static PartInViewController *_sharedManager = nil;
    dispatch_once(&onceToken, ^{
        _sharedManager = [PartInViewController new];
    });
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        // 注册自定义cell
        [self.tableView registerClass:[IndexTableViewCell class] forCellReuseIdentifier:@"activitycelll"];
...
-(void)loadData{
    ...
    NSURL *url = [NSURL URLWithString: [NSString stringWithFormat: KActivity_JoinActivities_URL,pageIndex,uid,token,latitude,longitude ] ];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url ];
    request.timeoutInterval=KHTTPTimeoutInterval;
    [request setHTTPMethod:@"GET"];
    
    __block PartInViewController *weakSelf = self;
...
{
    IndexTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"activitycelll" forIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
   ...
   NSDictionary *dic= self.arry_data[indexPath.row];
   ...
   NSString *url= [dic objectForKey:@"url"];
    ...
    url=[NSString stringWithFormat:@"%@&uid=%@&token=%@&lat=%@&long=%@",url,uid,token,latitude,longitude];
    UIWebViewViewController*webview=[[UIWebViewViewController alloc]init];
    [webview setUrlStr:url ];
